...
export default async function PrinterLayout({
  ...
  return (
    <CalendarClientWrapper>
      <SidebarProvider>
        <GenericAppSidebar title="Printer Portal" rootPath="/printer_dashboard" role="printer" footerText="Printing Operations" />
        <div className="flex-1 flex flex-col min-h-screen">
          <header className="h-16 flex items-center justify-between px-4 border-b bg-white/50 backdrop-blur-md sticky top-0 z-30">
            ...
            <div className="flex items-center gap-2">
              ...
              <UserMenu name={session?.name} role={session?.role} basePath="/printer_dashboard" />
            </div>
          </header>
