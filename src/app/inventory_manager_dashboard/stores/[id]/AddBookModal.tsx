...
import { searchBooks, getBookEditionsForTransfer, transferToStore } from '@/app/actions/transfer-actions';
...
export default function AddBookModal({ isOpen, onClose, storeId, storeName }: AddBookModalProps) {
    const [step, setStep] = useState(1); // 1: Search, 2: Editions
    ...
    const handleSelectBook = async (book: any) => {
        ...
        try {
            ...
            if (res.success) {
                ...
                console.error("Fetch editions res error:", res.error);
                toast.error(String(res.error));
            }
        } catch (err) {
            console.error("Fetch editions catch error:", err);
    ...
    const handleTransfer = async () => {
        ...
        const res = await transferToStore(storeId, transfers);
        if (res.success) {
            toast.success("Books transferred successfully");
            onClose();
            // Reset state
            setStep(1);
            setQuantities({});
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div className="space-y-0.5 overflow-hidden">
                            <DialogTitle className="text-2xl md:text-3xl font-black text-primarycolor uppercase italic truncate">
                                Add Books <span className="text-secondarycolor not-italic">to {storeName}</span>
                            </DialogTitle>
