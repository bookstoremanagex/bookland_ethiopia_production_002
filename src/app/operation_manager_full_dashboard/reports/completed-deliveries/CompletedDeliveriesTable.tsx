...
const useColumns = (formatDate: (date: Date) => string): ColumnDef<any>[] => [
  ...
  },
  {
    ...
    cell: ({ row }) => (
        <span className="font-black text-primarycolor text-sm">
            {row.getValue<number>("quantity").toLocaleString()} <span className="text-[10px] opacity-40">PCS</span>
        </span>
    ),
  },
  {
    ...
    cell: ({ row }) => (
        <div className="flex flex-col">
            ...
            <div className="flex items-center gap-1.5 mt-1">
                ...
                <span className="text-[9px] font-black text-emerald-600/60 uppercase tracking-widest">Fully Paid</span>
            </div>
        </div>
    ),
  },
  ...
  },
  {
    id: "actions",
    cell: ({ row }) => (
        <Link href={`/operation_manager_full_dashboard/reports/completed-deliveries/${row.original.id}`}>
            <Button variant="ghost" size="icon" className="rounded-full hover:bg-emerald-500 hover:text-white transition-all shadow-sm border border-slate-100">
  ...
  return (
    <div className="w-full space-y-6">
        <div className="flex items-center gap-6 px-10 h-20 bg-white rounded-[2rem] border-2 border-primarycolor/5 shadow-xl">
            ...
            />
            <div className="px-4 py-2 rounded-xl bg-emerald-50 border border-emerald-100 text-[10px] font-black text-emerald-600 uppercase tracking-widest shrink-0">
                {data.length} Settlements
            </div>
        </div>
