...
import PaymentsDueTable from "@/app/admin_dashboard/finance/payments-due/PaymentsDueTable";

export default async function DeliveryPaymentsDuePage() {
    ...
    const data = (shops as any[]).map((shop) => {
        ...
        const approvedPrevPaid = (shop.payments || []).reduce((s: number, p: any) => s + (p.amount || 0), 0);
    ...
    const totalLastOrderDebt = data.reduce((sum, s) => sum + s.lastOrderDebt, 0);
    const totalDebt = totalOrderDebt + totalRoundDebt + totalPreviousDebt + totalLastOrderDebt;
    ...
    return (
        <div className="p-4 md:p-10 space-y-8 bg-[#F8FAFC] min-h-screen">
            ...
            <PaymentsDueTable data={data} managePaymentBasePath="/delivery_dashboard_full/manage_payment" shopBasePath="/delivery_dashboard_full/book_shops" />
        </div>
    );
}
