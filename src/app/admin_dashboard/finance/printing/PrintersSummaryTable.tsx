...
import type { PrinterSummaryRow } from "./page"

export const columns: ColumnDef<PrinterSummaryRow>[] = [
  {
    accessorKey: "name",
    header: ({ column }) => (
      <Button
    ...
    cell: ({ row }) => (
        <div className="flex items-center gap-3">
            ...
            <span className="font-black text-primarycolor uppercase text-xs">{row.getValue("name")}</span>
        </div>
    ),
  },
  {
  ...
  },
  {
    ...
    cell: ({ row }) => (
        <div className="font-bold text-emerald-600">
            {row.getValue<number>("totalPaid").toLocaleString()} <span className="text-[8px] opacity-40">ETB</span>
        </div>
    ),
  },
  {
    ...
    cell: ({ row }) => (
        <div className="font-black text-rose-500">
            {row.getValue<number>("totalRemaining").toLocaleString()} <span className="text-[8px] opacity-40">ETB</span>
        </div>
    ),
  },
  {
    ...
    accessorFn: (row) => row.totalCost > 0 ? (row.totalPaid / row.totalCost) * 100 : 0,
    cell: ({ row }) => {
        const cost = row.original.totalCost;
        const paid = row.original.totalPaid;
        const rate = cost > 0 ? (paid / cost) * 100 : 0;
        return (
            <span className={`inline-flex items-center px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-widest ${
                rate >= 100 ? "bg-emerald-50 text-emerald-600" : rate >= 50 ? "bg-amber-50 text-amber-600" : "bg-rose-50 text-rose-500"
            }`}>
                {rate.toFixed(0)}%
            </span>
        );
...
export default function PrintersSummaryTable({ data }: { data: PrinterSummaryRow[] }) {
  ...
  const grandTotalCost = data.reduce((s, r) => s + r.totalCost, 0);
  const grandTotalPaid = data.reduce((s, r) => s + r.totalPaid, 0);
  const grandTotalRemaining = data.reduce((s, r) => s + r.totalRemaining, 0);
  ...
  return (
    <div className="w-full space-y-6 bg-white rounded-[3rem] p-8 md:p-10 border-2 border-primarycolor/5 shadow-xl">
      <div className="flex flex-col sm:flex-row sm:items-center gap-4 pb-4 border-b border-slate-100">
        <div className="flex items-center gap-3">
          <Printer className="size-5 text-secondarycolor" />
          <h2 className="text-sm font-black uppercase tracking-widest text-primarycolor">Printers Summary</h2>
        </div>
