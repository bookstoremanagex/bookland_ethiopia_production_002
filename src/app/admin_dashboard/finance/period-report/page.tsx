...
export default function PeriodReportPage() {
    ...
    const sevenDaysAgoISO = toISODateString(new Date(Date.now() - 6 * 86400000));
    ...
    const fetchData = useCallback(async (start: string, end: string) => {
        ...
        setLoading(true);
        try {
            const result = await getPeriodReportData(start, end);
            setData(result);
        } finally {
            setLoading(false);
        }
    ...
    const handleGenerate = () => fetchData(startDate, endDate);

    const displayStart = startDate ? new Date(startDate + "T12:00:00") : new Date();
    const displayEnd = endDate ? new Date(endDate + "T12:00:00") : new Date();
    const startLabel = formatLong(displayStart);
    const endLabel = formatLong(displayEnd);
...
<body>
    <h1>Period Report</h1>
    <div class="date-row">${escapeHtml(startLabel)} — ${escapeHtml(endLabel)}</div>
    ...
    return (
        <div className="p-3 md:p-10 space-y-4 md:space-y-8 bg-[#F8FAFC] min-h-screen">
            <h1 className="text-2xl md:text-4xl font-semibold tracking-tight text-slate-900">
                Period <span className="text-secondarycolor not-italic">Report</span>
            </h1>
            ...
            <div className="bg-white rounded-xl md:rounded-2xl border border-slate-200 p-4 md:p-5 space-y-4 md:space-y-5 shadow-sm">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                    <div className="flex items-center gap-2 min-w-0">
                        ...
                        {dateError ? (
                            <span className="text-sm font-bold text-red-500 truncate">Invalid date range</span>
                        ) : (
                            <span className="text-sm font-bold text-muted-foreground truncate">{startLabel} — {endLabel}</span>
                        )}
                    </div>
                    <div className="flex items-center gap-2">
                        <button
                            onClick={handleGenerate}
                            className="h-8 px-3.5 rounded-lg bg-primarycolor text-white font-black text-[10px] uppercase tracking-widest shadow-sm hover:bg-primarycolor/90 transition-colors flex items-center gap-1.5"
                        >
            ...
            {dateError ? (
                <div className="flex flex-col items-center justify-center py-12 md:py-20 text-muted-foreground">
                    ...
                    <p className="text-base md:text-lg font-bold uppercase tracking-widest text-red-400">Invalid date range</p>
                    <p className="text-[10px] font-bold uppercase tracking-widest mt-1">Please select valid start and end dates (start before end)</p>
                </div>
            ) : loading ? (
                ...
                <div className="flex flex-col items-center justify-center py-12 md:py-20 text-muted-foreground">
                    ...
                    <p className="text-base md:text-lg font-bold uppercase tracking-widest">No report generated yet</p>
                    <p className="text-[10px] font-bold uppercase tracking-widest mt-1">Select a date range and click Generate</p>
                </div>
            )}
            ...
            {data && data.ordersCount === 0 && data.roundsCount === 0 && data.paymentsCount === 0 && (
                <div className="flex flex-col items-center justify-center py-12 md:py-20 text-muted-foreground">
                    <CalendarDays className="size-12 md:size-16 mb-4 opacity-20" />
                    <p className="text-base md:text-lg font-bold uppercase tracking-widest">No activity for this period</p>
