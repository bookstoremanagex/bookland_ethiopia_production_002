...
export interface BookRevenueData {
    id: number
    title: string
    author: string
    totalCost: number
...
export const columns: ColumnDef<BookRevenueData>[] = [
  {
    accessorKey: "title",
    header: ({ column }) => (
        <Button
    ...
    cell: ({ row }) => (
        <div className="flex items-center gap-3">
            <div className="size-10 rounded-xl bg-primarycolor/5 flex items-center justify-center text-primarycolor border border-primarycolor/10">
                <BookOpen className="size-5 opacity-40" />
            </div>
            <div>
                <div className="font-black text-primarycolor uppercase text-xs tracking-tight">{row.getValue("title")}</div>
                <div className="flex items-center gap-1.5 mt-0.5">
                    <User className="size-2.5 text-muted-foreground/50" />
                    <span className="text-[9px] font-bold text-muted-foreground uppercase tracking-widest">{row.original.author}</span>
                </div>
            </div>
        </div>
    ),
  ...
  },
  {
    ...
    cell: ({ row }) => {
        const profit = row.getValue<number>("netProfit");
        return (
            <div className={`px-4 py-2 rounded-xl text-center inline-block min-w-[110px] border-2 ${
                profit >= 0
...
export default function BookRevenueTable({ data }: { data: BookRevenueData[] }) {
  ...
  const globalProfit = data.reduce((acc, curr) => acc + curr.netProfit, 0);
