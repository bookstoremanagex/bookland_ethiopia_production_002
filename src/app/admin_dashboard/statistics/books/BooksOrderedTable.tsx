...
export default function BooksOrderedTable({
    initialData,
}: {
    initialData: BookEntry[];
}) {
    ...
    const handleRangeChange = useCallback(async (value: string) => {
        ...
        setLoading(true);
        try {
            const result = await getOrderedBooks(value);
            setData(result);
        } finally {
            setLoading(false);
        }
    ...
    const filtered = useMemo(() => {
        ...
        return data.filter((b) => b.title.toLowerCase().includes(q));
    }, [data, search]);

    const totals = useMemo(
        () => ({
            qty: data.reduce((s, b) => s + b.totalQty, 0),
            price: data.reduce((s, b) => s + b.totalPrice, 0),
        }),
        [data]
    );

    const columns = useMemo<ColumnDef<BookEntry>[]>(
        () => [
            {
                id: "title",
                header: "Book",
            ...
            },
            {
                ...
                cell: ({ row }) => {
                    const date = new Date(row.original.latestOrderAt);
                    return (
            ...
            {loading ? (
                ...
                <div className="flex flex-col items-center justify-center py-20 text-muted-foreground">
                    ...
                    <p className="font-bold text-xs uppercase tracking-widest">
                        {data.length === 0
                            ? "No books ordered in this period"
