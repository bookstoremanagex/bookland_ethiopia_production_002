                        ...
                        <div className="grid grid-cols-1 gap-4 p-4 md:hidden">
                            {table.getRowModel().rows.length > 0 ? (
                                table.getRowModel().rows.map(row => {
                                    ...
                                    else if (status === "Pending") statusClass = "bg-amber-50 text-amber-700 border-amber-200/50"
                                    return (
                                        <div key={item.id} className="bg-white rounded-2xl border-2 border-primarycolor/5 p-5 space-y-4 hover:shadow-md transition-all">
                                            <div className="flex items-start justify-between gap-3">
                                                <div className="min-w-0 flex-1">
                                                    <div className="flex items-center gap-2">
                                                        <span className="text-[10px] font-black text-primarycolor/50">CON-{String(item.id).padStart(4, "0")}</span>
                                                        <span className={cn("inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full border text-[8px] font-black uppercase tracking-wider", statusClass)}>
                                                            {status === "Active" ? <CheckCircle2 className="size-2.5" /> : status === "Pending" ? <Clock className="size-2.5 animate-pulse" /> : <AlertCircle className="size-2.5" />}
                                                            {status}
                                                        </span>
                                                    </div>
                                            ...
                                            <div className="flex items-center gap-2">
                                                ...
                                                <span className="font-semibold text-xs text-secondarycolor/80 truncate">{item.party}</span>
                                            </div>

                                            <div className="flex flex-wrap gap-x-4 gap-y-1.5 text-xs">
                                                <span className="font-bold text-secondarycolor/60">{item.type}</span>
                                                {item.value && (
                                                    <span className="font-black text-secondarycolor">${item.value.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                                                )}
                                                {item.dateSigned && (
                                                    <span className="font-semibold text-secondarycolor/70 flex items-center gap-1">
                                                        <Calendar className="size-3 text-primarycolor/60" /> {formatDate(new Date(item.dateSigned))}
                                                    </span>
                                                )}
                                            </div>

                                            <div className="flex items-center gap-2 pt-2 border-t border-slate-100">
                                                <Link href={`/admin_dashboard/document_management/contracts/${item.id}`} className="flex-1">
