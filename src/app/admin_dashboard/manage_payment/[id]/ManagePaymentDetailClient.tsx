...
export default function ManagePaymentDetailClient({ shop, payments, orders, roundRecords, roundPayments, totals, previousDebt, roundBooksTotals }: Props) {
    ...
    // Payment Info stats — counts ALL payments in the history, categorized by source
    // (matches the Payment History list exactly: every item tagged "Order" or "Round")
    const paymentInfoStats = useMemo(() => {
        const orderItems = allPayments.filter((p) => p.source === "order");
        const roundItems = allPayments.filter((p) => p.source === "round");
        const sumOf = (arr: AllPaymentItem[]) => arr.reduce((s, p) => s + (p.amount || 0), 0);
        return {
            orderCount: orderItems.length,
            orderAmount: sumOf(orderItems),
            roundCount: roundItems.length,
            roundAmount: sumOf(roundItems),
            totalCount: allPayments.length,
            totalAmount: sumOf(allPayments),
        };
    }, [allPayments]);
                    ...
                    <div className="hidden sm:block space-y-4">
                        ...
                        {(() => {
                            return (
                                <div className="space-y-3">
                                    <div className="flex items-center justify-between py-2 px-3 rounded-xl bg-slate-100 border border-slate-200">
                                        <span className="text-[10px] font-bold text-slate-600">All Payments</span>
                                        <div className="text-right">
                                            <span className="text-sm font-black text-slate-800">{paymentInfoStats.totalCount} payments</span>
                                            <span className="text-[9px] font-bold text-slate-500 ml-2">{formatAmount(paymentInfoStats.totalAmount)} ETB</span>
                                        </div>
                                    </div>
                                    <div className="flex items-center justify-between py-2 px-3 rounded-xl bg-emerald-50 border border-emerald-100">
                                        <span className="text-[10px] font-bold text-emerald-600">Order Payments</span>
                                        <div className="text-right">
                                            <span className="text-sm font-black text-emerald-700">{paymentInfoStats.orderCount} payments</span>
                                            <span className="text-[9px] font-bold text-emerald-500 ml-2">{formatAmount(paymentInfoStats.orderAmount)} ETB</span>
                                        </div>
                                    </div>
                                    <div className="flex items-center justify-between py-2 px-3 rounded-xl bg-indigo-50 border border-indigo-100">
                                        <span className="text-[10px] font-bold text-indigo-600">Round Payments</span>
                                        <div className="text-right">
                                            <span className="text-sm font-black text-indigo-700">{paymentInfoStats.roundCount} payments</span>
                                            <span className="text-[9px] font-bold text-indigo-500 ml-2">{formatAmount(paymentInfoStats.roundAmount)} ETB</span>
                                        </div>
                                    </div>
                                </div>
                            )
                ...
                <div className="lg:col-span-3 bg-white rounded-[2rem] border-2 border-primarycolor/5 p-0 sm:p-6 shadow-xl">
                    <div className="hidden sm:block space-y-4">
                        <div className="flex items-center gap-2 text-rose-700 mb-1">
                            ...
                            <h3 className="text-[10px] font-black uppercase tracking-widest">Debt Breakdown</h3>
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                            ...
                            <div className="flex items-center justify-between py-2 px-3 rounded-xl bg-purple-50 border border-purple-100">
                                <span className="text-[10px] font-bold text-purple-600">Rounds</span>
                                <span className="text-sm font-black text-purple-700">{formatAmount(debtBreakdown.roundsDebt)} ETB</span>
                            </div>
                        </div>
                        <div className="space-y-2 rounded-2xl border-2 border-slate-100 bg-slate-50/50 p-4">
                            {[
                                ...
                                <div key={row.label} className="flex items-center justify-between py-1.5 px-3 rounded-xl bg-white border border-slate-100">
                                    ...
                                    <span className="text-sm font-black text-primarycolor">{formatAmount(row.value)} ETB</span>
                                </div>
                            ))}
                            <div className="flex items-center justify-between py-2 px-3 rounded-xl bg-rose-50 border border-rose-200">
                                <span className="text-[10px] font-black uppercase tracking-widest text-rose-600">Total Debt (Sum)</span>
                                <span className="text-base font-black text-rose-700">{formatAmount(debtBreakdown.totalDebt)} ETB</span>
                            </div>
                        </div>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            ...
                            <div className={cn(
                                ...
                                debtBreakdown.remaining > 0
                                    ? "bg-rose-50 border-rose-200"
                                    : "bg-emerald-50 border-emerald-200"
                            )}>
                                <span className="text-[10px] font-bold text-muted-foreground">Remaining (Total Debt - Total Paid)</span>
                                <span className={cn(
                                    "text-sm font-black",
                                    debtBreakdown.remaining > 0 ? "text-rose-700" : "text-emerald-700"
                                )}>
                                    {formatAmount(debtBreakdown.remaining)} ETB
                                </span>
                            </div>
                        </div>
                        <Link
                            href={`/admin_dashboard/manage_payment/${shop.id}/debts-breakdown`}
                            className="flex items-center justify-center gap-2 h-11 w-full rounded-2xl bg-primarycolor hover:bg-secondarycolor text-white font-black text-[10px] uppercase tracking-widest shadow-lg shadow-primarycolor/20 transition-all active:scale-[0.98]"
                        >
                            <BarChart3 className="size-4" />
                            Details
                        </Link>
                    ...
                    <div className="sm:hidden">
                        <Accordion type="single" collapsible>
                            <AccordionItem value="debt-breakdown" className="border-0">
                                <AccordionTrigger className="px-5 py-4 hover:no-underline [&>svg]:text-rose-600">
                                    <div className="flex items-center gap-2 text-rose-700">
                                        <BarChart3 className="size-4" />
                                        <span className="text-[10px] font-black uppercase tracking-widest">Debt Breakdown</span>
                                    </div>
                                </AccordionTrigger>
                                <AccordionContent className="px-5 pb-5">
                                    <div className="space-y-2.5">
                                        ...
                                        <Link
                                            href={`/admin_dashboard/manage_payment/${shop.id}/debts-breakdown`}
                                            className="flex items-center justify-center gap-2 h-11 w-full rounded-2xl bg-primarycolor hover:bg-secondarycolor text-white font-black text-[10px] uppercase tracking-widest shadow-lg shadow-primarycolor/20 transition-all active:scale-[0.98] mt-1"
                                        >
                                            <BarChart3 className="size-4" />
                                            Details
                                        </Link>
