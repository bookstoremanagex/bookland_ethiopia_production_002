...
export default async function PrintingBooksListPage() {
    ...
    for (const order of orders) {
        for (const payment of (order.printorder_payments || [])) {
            if (payment.reference) {
                ...
                if (match) {
                    const label = match[1];
                    editionPaidMap[label] = (editionPaidMap[label] || 0) + (parseFloat(payment.amount) || 0);
                }
            }
        }
    }
    ...
    const items = orders.flatMap((order: any) =>
        (order.printorder_items || []).map((item: any) => {
            ...
            const bookTitle = item.bookedition?.books?.title || "Unknown Book";
            const editionName = item.bookedition?.edition_name || "Unknown Edition";
        ...
        <div className="p-4 md:p-10 space-y-10 bg-[#F8FAFC] min-h-screen">
            <div className="space-y-2">
                <h1 className="text-3xl font-semibold tracking-tight text-slate-900 sm:text-4xl">
                    Printing <span className="text-secondarycolor not-italic">Books List</span>
                </h1>
                <p className="text-muted-foreground font-bold uppercase tracking-widest text-[10px]">
                    All books across every print project
                </p>
            </div>
            <PrintingBooksListTable items={items} />
        </div>
    );
}
