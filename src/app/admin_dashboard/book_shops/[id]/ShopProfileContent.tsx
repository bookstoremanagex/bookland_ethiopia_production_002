...
import { updateBookShop, deleteBookShop } from "@/app/actions/book-shop-actions";
...
export default function ShopProfileContent({ shop: initialShop }: ShopProfileContentProps) {
        ...
        <div className="min-h-screen bg-[#F8FAFC] p-4 md:p-10 space-y-8 md:space-y-10">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <Link href={`${dashboardRoot}/book_shops`}>
                    <Button variant="ghost" className="rounded-2xl gap-3 font-bold text-muted-foreground hover:text-primarycolor p-0 md:p-4 h-auto md:h-10">
                        <ArrowLeft className="size-4 md:size-5" /> <span className="text-sm md:text-base">Back to Shops</span>
                    </Button>
                </Link>
                <div className="flex flex-col md:flex-row items-stretch md:items-center gap-3">
                    ...
                    >
                        ...
                        <span className="md:hidden text-[9px] font-black uppercase tracking-widest">Delete Shop</span>
                    </Button>
                    <Button 
                        onClick={handleSave} 
                        disabled={isUpdating}
                    ...
                    >
                        <Save className="size-4 md:size-5" /> {isUpdating ? 'Saving...' : 'Save Changes'}
                    </Button>
                </div>
            </div>
                    ...
                    <div className="bg-white rounded-[1.8rem] md:rounded-[3rem] p-6 md:p-10 border-2 border-primarycolor/5 shadow-2xl relative overflow-hidden">
                        ...
                        <div className="relative space-y-8 md:space-y-10">
                            <div className="flex items-center gap-4 md:gap-6">
                                ...
                                <div>
                                    <h1 className="text-2xl md:text-4xl font-black text-primarycolor uppercase tracking-tighter italic">Shop <span className="text-secondarycolor not-italic">Identity</span></h1>
                                    <p className="text-[8px] md:text-[10px] font-bold text-muted-foreground uppercase tracking-widest">Update partner information</p>
                                </div>
                            </div>

                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
                                <div className="space-y-2 md:space-y-3">
                                    <label className="text-[9px] md:text-[10px] font-black text-muted-foreground uppercase tracking-widest ml-4">Shop Name</label>
                                ...
                                <div className="space-y-2 md:space-y-3">
                                    <label className="text-[9px] md:text-[10px] font-black text-muted-foreground uppercase tracking-widest ml-4">Branch / Tag</label>
                                    <div className="relative group">
                                ...
                                <div className="space-y-2 md:space-y-3 md:col-span-2">
                                    <label className="text-[9px] md:text-[10px] font-black text-muted-foreground uppercase tracking-widest ml-4">Location / Address</label>
                                    <div className="relative group">
                                ...
                                <div className="space-y-2 md:space-y-3">
                                    <label className="text-[9px] md:text-[10px] font-black text-muted-foreground uppercase tracking-widest ml-4">Contact Phone</label>
                                    <div className="relative group">
                                ...
                                <div className="space-y-2 md:space-y-3">
                                    <label className="text-[9px] md:text-[10px] font-black text-muted-foreground uppercase tracking-widest ml-4">Email Address</label>
                                    <div className="relative group">
                ...
                <div className="space-y-6 md:space-y-8">
                    <div className="bg-primarycolor rounded-[1.8rem] md:rounded-[3rem] p-8 md:p-10 text-white shadow-2xl shadow-primarycolor/30 space-y-8 md:space-y-10 relative overflow-hidden">
                        ...
                        <div className="space-y-1 md:space-y-2 relative">
                            <p className="text-[9px] md:text-[10px] font-black uppercase tracking-widest opacity-60">Account Maturity</p>
                            <h3 className="text-lg md:text-xl font-bold">Partner since {new Date(shop.createdAt).toLocaleDateString()}</h3>
                        </div>
                        ...
                        <div className="space-y-5 md:space-y-6 relative">
                            <div className="flex items-center gap-3 md:gap-4">
                                ...
                                <div>
                                    <p className="text-[9px] md:text-[10px] font-black uppercase tracking-widest opacity-60 leading-none">Distributed Value</p>
                                    <p className="text-xl md:text-2xl font-black mt-1">
                                        {(shop.bookshopeditions || []).reduce((acc: any, ed: any) => acc + (ed.total_price || 0), 0).toLocaleString()} <span className="text-[10px] opacity-60">ETB</span>
                                    </p>
                                </div>
                            </div>
                            ...
                            <div className="flex items-center gap-3 md:gap-4">
                                ...
                                <div>
                                    <p className="text-[9px] md:text-[10px] font-black uppercase tracking-widest opacity-60 leading-none">Global Units</p>
                                    <p className="text-xl md:text-2xl font-black mt-1">
                                        {(shop.bookshopeditions || []).reduce((acc: any, ed: any) => acc + (ed.quantity || 0), 0).toLocaleString()} <span className="text-[10px] opacity-60 font-bold uppercase tracking-widest ml-1">Units</span>
                                    </p>
                                </div>
                            </div>
                            ...
                            <div className="flex items-center gap-3 md:gap-4">
                                ...
                                <div>
                                    <p className="text-[9px] md:text-[10px] font-black uppercase tracking-widest opacity-60 leading-none">Unpaid Debt</p>
                                    <p className="text-xl md:text-2xl font-black mt-1 text-rose-100">
                                        {(shop.bookshopeditions || []).reduce((acc: any, ed: any) => acc + (ed.remaining_amount || 0), 0).toLocaleString()} <span className="text-[10px] opacity-60">ETB</span>
                                    </p>
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="bg-white rounded-[1.8rem] md:rounded-[3rem] p-8 md:p-10 border-2 border-primarycolor/5 shadow-xl space-y-4 md:space-y-6">
                        <h4 className="text-[9px] md:text-[10px] font-black text-muted-foreground uppercase tracking-widest italic">Management Info</h4>
                        <div className="space-y-3 md:space-y-4">
                            <div className="flex items-center justify-between py-2 md:py-3 border-b border-slate-50">
                                <span className="text-[9px] md:text-[10px] font-black uppercase tracking-widest opacity-40">Shop ID</span>
                                <span className="font-black text-primarycolor text-sm md:text-base">#{shop.id}</span>
                            </div>
                            <div className="flex items-center justify-between py-2 md:py-3 border-b border-slate-50">
                                <span className="text-[9px] md:text-[10px] font-black uppercase tracking-widest opacity-40">Last Activity</span>
                                <span className="font-black text-primarycolor text-sm md:text-base">{new Date(shop.updatedAt).toLocaleDateString()}</span>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div className="bg-white rounded-[1.8rem] md:rounded-[3rem] p-6 md:p-10 border-2 border-primarycolor/5 shadow-xl space-y-6 md:space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div className="flex items-center gap-3 md:gap-4 text-primarycolor">
                        ...
                        <h3 className="text-xl md:text-2xl font-black uppercase tracking-tight italic">Active <span className="text-secondarycolor not-italic">Assignments</span></h3>
                    </div>
                ...
                <div className="w-full">
                    {(shop.bookshopeditions || []).length > 0 ? (
                        ...
                        <div className="text-center py-16 md:py-20 bg-slate-50/50 rounded-[1.5rem] md:rounded-[2rem] border-2 border-dashed border-slate-100">
                            ...
                            <p className="text-[8px] md:text-[10px] font-black text-slate-400 uppercase tracking-widest">No books currently assigned</p>
                        </div>
                    )}
                </div>
            </div>
