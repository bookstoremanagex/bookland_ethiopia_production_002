...
} from "@/app/actions/store-inventory-actions";
...
export function StoreInventoryTable({ data }: { data: InventoryItem[] }) {
  ...
  const onConfirmDelete = async () => {
    ...
    try {
      ...
      const newRemaining =
        currentEdition.count_remening_for_transfer + selectedItem.quantity;

      // 1. Return quantity to edition remaining
      ...
      const res = await deleteStoreInventory(
        selectedItem.id,
        selectedItem.bookedition.id,
      );
      if (res.success) {
        toast.success("Item removed from store and quantity returned to stock");
        router.refresh();
              ...
              {table.getHeaderGroups().map((headerGroup) => (
                ...
                >
                  {headerGroup.headers.map((header) => (
                    <TableHead
                      key={header.id}
                      className="h-16 px-8 text-[10px] font-black uppercase tracking-[0.2em] text-primarycolor/40"
                    >
          ...
          <DialogHeader className="space-y-4">
            ...
            <div>
              <DialogTitle className="text-2xl font-black text-primarycolor uppercase italic">
                Adjust{" "}
                <span className="text-secondarycolor not-italic">Stock</span>
              </DialogTitle>
          ...
          <div className="space-y-6 py-6">
            <div className="space-y-2">
              ...
              <p className="text-[9px] font-bold text-primarycolor uppercase tracking-widest ml-1">
                Stock Remaining for transfer:{" "}
                {selectedItem?.bookedition.count_remening_for_transfer} Units
              </p>
            </div>
          </div>
        ...
        <DialogContent className="rounded-[2rem] max-w-md p-8">
          <DialogHeader className="space-y-4">
            ...
            <div>
              ...
              <DialogDescription className="font-bold text-xs uppercase tracking-widest mt-1">
                This will remove {selectedItem?.quantity} units from this store.
              </DialogDescription>
            </div>
          </DialogHeader>
          ...
          <div className="bg-rose-50 p-6 rounded-2xl border-2 border-rose-100/50">
            <p className="text-xs font-bold text-rose-700 leading-relaxed italic">
              Are you sure? This will return all units back to the central stock
              inventory.
            </p>
          </div>
