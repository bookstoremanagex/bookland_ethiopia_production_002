...
const EditableField = ({ label, field, value, icon: Icon, type = "number", suffix = "", editValues, setEditValues, handleSaveEdit, isUpdating }: any) => {
  return (
    <div className="group relative bg-white p-6 md:p-8 rounded-[1.8rem] md:rounded-[2.5rem] border-2 border-primarycolor/5 shadow-xl transition-all hover:border-primarycolor/20">
      <div className={`size-10 md:size-12 rounded-xl md:rounded-2xl flex items-center justify-center mb-4 ${field === 'already_paid' ? 'bg-emerald-50 text-emerald-600' : 'bg-slate-100 text-slate-600'}`}>
      ...
      <div className="space-y-1">
        ...
        <div className="flex items-center justify-between group/val">
          <h4 className="text-2xl md:text-3xl font-black text-primarycolor">
              {type === 'number' ? Number(value).toLocaleString() : value} <span className="text-xs md:text-sm">{suffix}</span>
          </h4>
    ...
    <div className="min-h-screen bg-[#F8FAFC] p-4 md:p-10 space-y-10">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <Link href={`${dashboardRoot}/book_shops`}>
          <Button variant="ghost" className="rounded-2xl gap-3 font-bold text-muted-foreground hover:text-primarycolor p-0 h-auto md:p-4 md:h-10">
            <ArrowLeft className="size-4 md:size-5" /> <span className="text-xs md:text-sm">Back to Shops</span>
          </Button>
        </Link>
                ...
                <div className="flex flex-col md:flex-row md:items-center gap-6 md:gap-8">
                    ...
                    <div>
                        ...
                        <p className="text-sm md:text-xl font-bold text-muted-foreground mt-1 md:mt-2 uppercase tracking-widest flex items-center gap-2">
                            <span className="text-secondarycolor">{assignment.bookshopes.branch || 'Main'}</span> Branch
                        </p>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="p-6 md:p-8 rounded-[1.8rem] md:rounded-[2.5rem] bg-slate-50 border-2 border-white shadow-inner space-y-3 md:space-y-4">
                        <div className="flex items-center gap-2 md:gap-3 text-primarycolor">
                            ...
                            <span className="font-black uppercase tracking-widest text-[10px]">Content</span>
                        </div>
                        <div>
                            <h3 className="text-lg md:text-2xl font-black text-primarycolor uppercase line-clamp-2">{assignment.bookedition.books.title}</h3>
                            <p className="text-[10px] md:text-sm font-bold text-muted-foreground uppercase tracking-widest mt-1">{assignment.bookedition.edition_name}</p>
                        </div>
                    </div>
                    ...
                    <div className="p-6 md:p-8 rounded-[1.8rem] md:rounded-[2.5rem] bg-primarycolor text-white shadow-xl shadow-primarycolor/20 space-y-3 md:space-y-4">
                        <div className="flex items-center gap-2 md:gap-3 opacity-60">
                            ...
                            <span className="font-black uppercase tracking-widest text-[10px]">Total Revenue</span>
                        </div>
                        <div>
                            <h3 className="text-3xl md:text-4xl font-black">{mounted ? assignment.total_price?.toLocaleString() : '...'} <span className="text-lg md:text-xl opacity-60">ETB</span></h3>
                            <p className="text-[8px] md:text-xs font-bold uppercase tracking-widest mt-1 md:mt-2 opacity-60">Wholesale Value</p>
                        </div>
                    </div>
                </div>
            </div>
