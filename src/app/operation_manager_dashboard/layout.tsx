...
export default async function OperationLayout({
  ...
  return (
    <SidebarProvider>
      <GenericAppSidebar title="Ops Command" rootPath="/operation_manager_dashboard" role="operation_manager" footerText="Operations Control" />
      <div className="flex-1 flex flex-col min-h-screen">
        <header className="h-16 flex items-center justify-between px-4 border-b bg-white/50 backdrop-blur-md sticky top-0 z-30">
          <SidebarTrigger />
          <UserMenu name={session?.name} role={session?.role} basePath="/operation_manager_dashboard" />
        </header>
