...
export default async function SalesLayout({
  ...
  return (
    <SidebarProvider>
      <GenericAppSidebar title="Sales Portal" rootPath="/sales_staff_dashboard" role="sales_staff" footerText="Sales & Customer Service" />
      <div className="flex-1 flex flex-col min-h-screen">
        <header className="h-16 flex items-center justify-between px-4 border-b bg-white/50 backdrop-blur-md sticky top-0 z-30">
          <SidebarTrigger />
          <UserMenu name={session?.name} role={session?.role} basePath="/sales_staff_dashboard" />
        </header>
