...
import { Store, Building2, Package, ArrowRight, ExternalLink } from 'lucide-react';
    ...
    <div className="space-y-6 md:space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="bg-white rounded-[1.5rem] md:rounded-[2.5rem] p-6 md:p-10 border-2 border-primarycolor/10 shadow-2xl space-y-6 md:space-y-10">
        <div className="flex flex-col md:flex-row md:items-center gap-4 md:gap-6">
          ...
          <div>
            <h2 className="text-2xl md:text-4xl font-black text-primarycolor uppercase tracking-tight italic">Store <span className="text-secondarycolor not-italic">Distribution</span></h2>
            <p className="text-muted-foreground font-bold tracking-tight text-xs md:text-base">Overview of where this book is currently stocked across all editions.</p>
          </div>
        </div>
        ...
        <div className="hidden md:block overflow-hidden rounded-3xl border-2 border-primarycolor/5 shadow-sm">
          <table className="w-full text-left border-collapse">
            <thead className="bg-primarycolor/5 border-b-2 border-primarycolor/5">
              <tr>
                <th className="p-6 text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground">Retail Store</th>
                <th className="p-6 text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground">Assigned Edition</th>
                <th className="p-6 text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground text-center">Current Stock</th>
                  ...
                  <tr key={assignment.id} className="group hover:bg-primarycolor/[0.02] transition-colors">
                    <td className="p-6">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <div className="font-black text-primarycolor uppercase tracking-tight">{assignment.stores.name}</div>
                          <div className="text-[10px] text-muted-foreground font-bold uppercase tracking-widest">{assignment.stores.location}</div>
                        </div>
                      </div>
                    </td>
                    <td className="p-6">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="font-bold text-primarycolor">{assignment.editionName}</span>
                      </div>
                    </td>
                    ...
                    <td className="p-6 text-right">
                      <Button asChild variant="ghost" className="h-12 px-6 rounded-xl hover:bg-primarycolor hover:text-white transition-all font-black uppercase tracking-widest text-[10px] gap-2">
                        <Link href={`${dashboardRoot}/books/editions/${assignment.editionId}`}>
                ...
                <tr>
                  <td colSpan={4} className="p-24 text-center">
                    <div className="flex flex-col items-center gap-6 opacity-30">
                      ...
                      <div>
                        <p className="text-xl font-black uppercase tracking-[0.2em] text-primarycolor">No Store Assignments</p>
                        <p className="font-bold text-muted-foreground">Navigate to an edition to assign it to retail stores.</p>
                      </div>
                    </div>
                  </td>
                </tr>
        ...
        <div className="grid grid-cols-1 gap-4 md:hidden">
          {allAssignments.length > 0 ? (
              ...
              <div key={assignment.id} className="p-5 rounded-2xl border-2 border-primarycolor/5 bg-primarycolor/[0.01] space-y-4">
                <div className="flex items-start justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="font-black text-primarycolor uppercase tracking-tight text-sm">{assignment.stores.name}</div>
                      <div className="text-[10px] text-muted-foreground font-bold uppercase tracking-widest">{assignment.stores.location}</div>
                    </div>
                  </div>
                ...
                <div className="flex items-center justify-between pt-3 border-t border-primarycolor/5">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-bold text-primarycolor text-[11px]">{assignment.editionName}</span>
                  </div>
            ...
            <div className="py-12 text-center opacity-30">
              ...
              <p className="font-black uppercase tracking-widest text-xs text-primarycolor text-center px-6">No Store Assignments</p>
            </div>
          )}
        </div>
      </div>
