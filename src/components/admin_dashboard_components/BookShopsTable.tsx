        ...
        return (
          <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-all duration-300 transform translate-x-2 group-hover:translate-x-0">
            ...
            <Button 
              size="icon" 
              variant="ghost" 
              className="size-10 rounded-xl hover:bg-rose-50 text-rose-500"
              onClick={() => onDelete(shop)}
            >
              <Trash2 className="size-4" />
            </Button>
            <Button asChild className="h-10 px-5 rounded-xl bg-primarycolor font-black uppercase tracking-widest text-[9px] gap-2 ml-1 shadow-md shadow-primarycolor/10">
              <Link href={`${dashboardRoot}/book_shops/${shop.id}`}>
          ...
          table.getRowModel().rows.map((row) => {
            ...
            return (
              ...
              >
                ...
                <div className="mt-6 md:mt-8 pt-4 md:pt-6 border-t-2 border-primarycolor/5 flex items-center justify-between gap-3 md:gap-4">
                    <div className="flex gap-1 md:gap-2">
                        <Button size="icon" variant="ghost" className="size-10 md:size-12 rounded-lg md:rounded-xl hover:bg-primarycolor/10 text-primarycolor" onClick={() => onEdit(shop)}>
                            <Edit2 className="size-4 md:size-5" />
                        </Button>
                        <Button size="icon" variant="ghost" className="size-10 md:size-12 rounded-lg md:rounded-xl hover:bg-rose-50 text-rose-500" onClick={() => onDelete(shop)}>
                            <Trash2 className="size-4 md:size-5" />
                        </Button>
                    </div>
                    <Button asChild className="flex-1 h-10 md:h-12 bg-primarycolor font-black uppercase tracking-widest text-[9px] md:text-[10px] rounded-lg md:rounded-xl gap-2 shadow-lg shadow-primarycolor/10">
                        <Link href={`${dashboardRoot}/book_shops/${shop.id}`}>
                            Details <ArrowRight className="size-3.5 md:size-4" />
                        </Link>
                    </Button>
                </div>
              </div>
