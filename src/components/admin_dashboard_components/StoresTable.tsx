...
import { Search, ChevronLeft, ChevronRight, Eye, Store, MapPin, Phone, Mail, Building2 } from "lucide-react";
      ...
      cell: ({ row }) => {
        ...
        return (
          <Button variant="ghost" size="icon" asChild className="size-10 hover:text-primarycolor hover:bg-primarycolor/10 rounded-full transition-all active:scale-90">
            <Link href={`${dashboardRoot}/stores/${store.id}`}>
  ...
  return (
    <div className="w-full space-y-8 animate-in fade-in duration-700">
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-6 bg-card p-6 rounded-2xl border-2 border-primarycolor/5 shadow-md transition-all duration-300 hover:shadow-xl hover:border-primarycolor/10">
        ...
        <Button className="w-full sm:w-auto h-12 px-8 bg-primarycolor hover:bg-secondarycolor text-white font-black rounded-2xl shadow-lg shadow-primarycolor/20 transition-all active:scale-95 flex items-center gap-2 group" asChild>
          <Link href={`${dashboardRoot}/stores/add_stores`}>
            <span className="text-xl transition-transform group-hover:rotate-90">+</span> Add Store
          </Link>
        </Button>
      </div>
