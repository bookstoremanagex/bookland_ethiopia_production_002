...
const DetailsCell = ({ id }: { id: number }) => {
  ...
  return (
    <Button asChild variant="ghost" size="icon" className="rounded-xl hover:bg-primarycolor hover:text-white transition-all shadow-sm">
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <div className="font-black text-primarycolor">
        {(row.getValue<number>("total_price") || 0).toLocaleString()} <span className="text-[10px] opacity-40">ETB</span>
      </div>
    ),
  },
  {
      ...
      return (
        <div className="space-y-1.5 min-w-[180px]">
          <div className="flex justify-between text-[9px] font-black uppercase tracking-widest mb-1">
            <span className="text-primarycolor/40">Value: {total.toLocaleString()}</span>
            <span className={cn(remaining > 0 ? "text-rose-500" : "text-emerald-500")}>
              Due: {(remaining || 0).toLocaleString()}
            </span>
          </div>
          <div className="flex justify-between text-[9px] font-black uppercase tracking-widest">
            <span className="text-emerald-500">Paid: {(paid || 0).toLocaleString()}</span>
          </div>
          <div className="h-1.5 w-full bg-slate-100 rounded-full overflow-hidden mt-1">
            <div 
              className={cn("h-full transition-all duration-1000", isFullyPaid ? "bg-emerald-500" : "bg-primarycolor")}
              style={{ width: `${Math.min(100, ((paid || 0) / (total || 1)) * 100)}%` }}
            />
          </div>
        </div>
      );
      ...
      <div className="md:hidden space-y-4">
        {table.getRowModel().rows?.length ? (
          table.getRowModel().rows.map((row) => {
            ...
            return (
              <div key={row.id} className="bg-white rounded-3xl border-2 border-slate-100 p-6 space-y-6 shadow-md">
                <div className="flex justify-between items-start">
                  <div>
                    <h4 className="font-black text-primarycolor leading-tight">{assignment.bookedition.books.title}</h4>
                    <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest mt-1">{assignment.bookedition.edition_name}</p>
                  </div>
                  <Button asChild variant="outline" size="icon" className="rounded-xl shrink-0">
                    <Link href={`${dashboardRoot}/shop_assignments/${assignment.id}`}>
                      <ExternalLink className="size-4" />
                    </Link>
                  </Button>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4 pt-4 border-t border-slate-50">
                  ...
                  <div>
                    ...
                    <div className="font-black text-primarycolor text-sm">
                      {total.toLocaleString()} <span className="text-[10px] opacity-40">ETB</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-2 pt-4 border-t border-slate-50">
                   <div className="flex justify-between text-[9px] font-black uppercase tracking-widest">
                      <span className="text-emerald-500">Paid: {paid.toLocaleString()}</span>
                      <span className={cn(remaining > 0 ? "text-rose-500" : "text-emerald-500")}>
                   ...
                   <div className="h-2 w-full bg-slate-100 rounded-full overflow-hidden">
                      <div 
                        className={cn("h-full transition-all duration-1000", remaining <= 0 ? "bg-emerald-500" : "bg-primarycolor")}
