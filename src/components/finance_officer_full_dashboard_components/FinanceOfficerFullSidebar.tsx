...
const menuItems = [
  { title: "Home", icon: Home, url: "/finance_officer_full_dashboard" },
  { title: "Notifications", icon: Bell, url: "/finance_officer_full_dashboard/notifications" },
  { title: "Notes", icon: FileText, url: "/finance_officer_full_dashboard/notes" },
  { title: "Profile", icon: User, url: "/finance_officer_full_dashboard/profile" },
  { title: "Finance Books", icon: BookOpen, url: "/finance_officer_full_dashboard/books" },
  { title: "Shop Table", icon: TableProperties, url: "/finance_officer_full_dashboard/shop-table" },
  { title: "Edition Table", icon: BookCopy, url: "/finance_officer_full_dashboard/edition-table" },
  { title: "Costs", icon: FileText, url: "/finance_officer_full_dashboard/costs" },
  { title: "Printing Finance", icon: Printer, url: "/finance_officer_full_dashboard/printing" },
];

export function FinanceOfficerFullSidebar() {
  ...
  const activeUrl = React.useMemo(() => {
    ...
    for (const item of sortedItems) {
      if (item.url === "/finance_officer_full_dashboard") {
        if (activePath === "/finance_officer_full_dashboard") return item.url;
        continue;
      }
  ...
  return (
    <TooltipProvider delayDuration={0}>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-50 flex items-center justify-around bg-white border-t-2 border-primarycolor/10 px-2 py-1 shadow-[0_-4px_20px_rgba(0,0,0,0.05)] md:hidden">
        <Link
          href="/finance_officer_full_dashboard"
          onClick={() => setOpenMobile(false)}
          className={cn(
            "flex flex-col items-center gap-0.5 py-1 px-4 rounded-xl transition-all",
            activePath === "/finance_officer_full_dashboard"
              ? "text-primarycolor"
